...
int main(){
  ...
  rep(i, 0, 1000){
    ...
    ll c = (ll)rand() << 2;
    ll k = (ll)rand() << 2;
    ll m = ((ll)rand() >> 2) + 1, a = t / 3;
    ll term = (c + (ll)((__int128)k*t % m)) % m;
    ll c2 = (c + (ll)((__int128)k*a % m)) % m;
    assert(modsum(t+1, c, k, m)-modsum(t, c, k, m)==term);
    assert(modsum(t, c, k, m)==modsum(a, c, k, m)+modsum(t-a, c2, k, m));
  }
