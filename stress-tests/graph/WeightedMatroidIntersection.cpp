...
#include "../../content/graph/WeightedMatroidIntersection.h"
...
int main(){
  {
    ...
    vi w{10, 5, 7, 6};
    WeightedMatroidIntersection mi(4, m1, m2);
    static_assert(is_same_v<decltype(mi.calc(w)), pair<int, vi>>);
    auto [weight, I] = mi.calc(w);
    assert(weight == 18 && sz(I) == 3);
    assert(independent(I, color, cap1) && independent(I, shape, cap2));
  }
  {
    vector<unsigned long long> a{8, 14, 6, 14, 7, 5, 0, 3, 4, 14};
    vector<unsigned long long> b{11, 5, 15, 10, 14, 8, 12, 5, 10, 4};
    auto oka = linearMatroid(a), okb = linearMatroid(b);
    vi w{3, 8, -10, 7, -6, 9, 4, 7, 8, 5};
    TableMatroid ma(oka), mb(okb);
    WeightedMatroidIntersection mi(10, asTuple(ma), asTuple(mb));
    auto [weight, I] = mi.calc(w);
    assert(weight == 29 && sz(I) == 4);
  }
  mt19937_64 rng(1);
  rep(tc, 0, 1000){
    ...
    auto oka = linearMatroid(a), okb = linearMatroid(b);
    vi w(n);
    rep(i, 0, n) w[i] = (int)(rng() % 21) - 10;
    int bestSize = -1, bestWeight = 0, bestMask = 0;
    rep(mask, 0, 1 << n) if(oka[mask] && okb[mask]){
      int cnt = __builtin_popcount(mask), sum = 0;
      rep(i, 0, n) if(mask >> i & 1) sum += w[i];
      if(cnt > bestSize || (cnt == bestSize && sum > bestWeight)){
        bestSize = cnt; bestWeight = sum; bestMask = mask;
      }
    }
    TableMatroid ma(oka), mb(okb);
    WeightedMatroidIntersection mi(n, asTuple(ma), asTuple(mb));
    auto [weight, I] = mi.calc(w);
    int mask = getMask(I, n);
    assert(oka[mask] && okb[mask]);
    if(sz(I) != bestSize || weight != bestWeight){
      cerr << "tc " << tc << " n " << n << " dim " << dim << '\n';
      cerr << "a "; for(auto x : a) cerr << x << ' '; cerr << '\n';
      cerr << "b "; for(auto x : b) cerr << x << ' '; cerr << '\n';
      cerr << "w "; for(int x : w) cerr << x << ' '; cerr << '\n';
      cerr << "got " << weight << ' '; for(int x : I) cerr << x << ' '; cerr << '\n';
      cerr << "expected " << bestWeight << " size " << bestSize << ' ';
      rep(i, 0, n) if(bestMask >> i & 1) cerr << i << ' ';
      cerr << '\n';
      return 1;
    }
  }
  {
    int k = 50, n = k * k;
    vi row(n), col(n), cap(k, 1), w(n);
    rep(i, 0, k) rep(j, 0, k){
      int e = i * k + j;
      row[e] = i; col[e] = j; w[e] = rng() % 1000000;
    }
    vi cnt1(k), cnt2(k);
    auto m1 = tuple(
      [&](const vi &I){
    ...
    auto [weight, I] = mi.calc(w);
    assert(weight >= 0 && sz(I) == k);
    assert(independent(I, row, cap) && independent(I, col, cap));
  }
  cout << "Tests passed!\n";
}
