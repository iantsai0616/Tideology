 ...
 * License: CC0
 * Source: own work
 * Description: Immediate dominators from source s. Unreachable nodes get -1.
 * Time: O((V+E) alpha(V))
 * Status: stress-tested against the definition
 */
#pragma once
struct DominatorTree {
  ...
  void build(int s){
    id = 0;
    for(auto &v : rg) v.clear();
    for(auto &v : bucket) v.clear();
    fill(all(vis), -1); fill(all(dom), -1);
    dfs(s);
    for(int i = id - 1; ~i; --i){
      for(int u : rg[i])
