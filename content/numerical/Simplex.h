...
#define MP make_pair
#define ltj(X) if(s == -1 or MP(X[j], N[j]) < MP(X[s], N[s])) s = j
...
struct LPSolver{
  ...
  void pivot(int r, int s){
    ...
    rep(i, 0, m + 2) if(i != r and abs(D[i][s]) > eps){
  ...
  bool simplex(int phase){
    ...
    for(;;){
      ...
      rep(i, 0, m){
        ...
        if(r == -1 or MP(D[i][n + 1] / D[i][s], B[i]) <
  ...
  pair<T, vd> solve(){
    ...
    if(D[r][n + 1] < -eps){
      pivot(r, n);
      if(!simplex(2) or D[m + 1][n + 1] < -eps) return {-inf, {}};
      rep(i, 0, m) if(B[i] == -1){
        int s = 0;
        rep(j, 1, n + 1) ltj(D[i]);
        pivot(i, s);
